...
export default function AddAssignment() {
  ...
  const [inputEndTime, setInputEndTime] = React.useState('');
  ...
  let currentPath = window.location.pathname.substr(31);
  ...
  const onSubmit = () =>{
    ...
    const endDate = inputEndDate.$D +'-'+Number(inputEndDate.$M+1)+'-'+inputEndDate.$y;
    const endTime = convertToDecimal(inputEndTime.$H) + ':'+ convertToDecimal(inputEndTime.$m);
    const assignDate =  today.getDate() + '-' + (today.getMonth() + 1) +'-'+ today.getFullYear();
    axios.post("http://localhost:3000/teacher/create/assignment",{title: inputText,desc: inputDesc, course_id: course_id, assignOn: assignDate, dueDate:dateline,endDate:endDate, endTime: endTime , dueTime: dueTime, score: inputScore, group: inputGroup, file: inputFile},{ withCredentials: true })
      .then((result) => {
        alert("Assignment Created Successfully!!");
        window.location.replace(`/teacher/course/assignment_list/${currentPath}`)
        })
  ...
  const handleClick = () =>{
    // alert("Assignment Discard Successfully!")
    Navigate(`/teacher/course/assignment_list/${currentPath}`);
  }
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer components={['MobileTimePicker']}>
                    ...
                    <DemoItem label="Time End *">
                      <MobileTimePicker
                        value={inputEndTime}
                        onChange={(newValue) => setInputEndTime(newValue)}
                      />
                    </DemoItem>
                  </DemoContainer>
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer components={['MobileTimePicker']}>
                    ...
                    <DemoItem label="Time Due *">
                      <MobileTimePicker
                        value={inputDueTime}
                        onChange={(newValue) => setInputDueTime(newValue)}
                      />
                    </DemoItem>
                  </DemoContainer>
                </LocalizationProvider>
